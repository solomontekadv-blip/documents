...
const headTable = new Table({
  ...
  rows: [new TableRow({ children: [
    ...
    cell([para([run("ס\"ע ________________", { bold: true })], { align: AlignmentType.END, after: 0 }),
          para("תאריך: 10.09.2026", { align: AlignmentType.END, after: 0 })], CW / 2),
  ]})]
});
...
const doc = new Document({
  ...
  sections: [{
    ...
    children: [
      ...
      para([run("סכום התביעה: ", { bold: true }), run("63,120 ₪.")], { after: 40 }),
      ...
      clause(["התובע זכאי לפדיון חופשה בסך ", { t: "6,909 ₪", bold: true }, ", לפי 21 ימים (18 ימים לשנה, באופן יחסי לימי העבודה בפועל) × 329 ₪, שכרו היומי הממוצע לפי סעיף 10 לחוק חופשה שנתית (92,533 ₪ שכר לכל התקופה / 281 ימי עבודה)."]),
      clause(["התובע זכאי לדמי חגים בסך ", { t: "2,578 ₪", bold: true }, ", לפי 9 ימים × 286.44 ₪."]),
      clause(["התובע זכאי לפיצוי לדוגמה בסך ", { t: "20,000 ₪", bold: true }, ": 10,000 ₪ בגין אי-מסירת הודעה על תנאי עבודה לעובד קטין, ו-10,000 ₪ בגין אי-מסירת 27 תלושי שכר לאורך כל תקופת העבודה, אשר הופקו לראשונה שנתיים לאחר סיומה."]),
      clause([{ t: "סיכום. ", bold: true }, "על הנתבעת לשלם לתובע את הסך ", { t: "63,120 ₪", bold: true }, ", בצירוף הפרשי הצמדה וריבית כחוק ממועד היווצרות כל אחד מהחיובים ועד התשלום בפועל."]),
