...
const doc = new Document({
  ...
  sections: [{
    ...
    children: [
      ...
      partiesTable,
      para("", { after: 40 }),
      para([run("מהות התביעה: ", { bold: true }), run("שכר עבודה, גמול שעות נוספות, פיצויי פיטורים, פדיון חופשה, דמי חגים, דמי מחלה ופיצוי לדוגמה (עובד קטין).")], { after: 40 }),
      para([run("סכום התביעה: ", { bold: true }), run("70,090 ₪.")], { after: 40 }),
      ...
      clause(["העסקת התובע הסתיימה ביום 28.9.2024 עקב גיוסו לשירות סדיר בצה\"ל. התפטרות עקב גיוס דינה כפיטורים לעניין פיצויי פיטורים, מכוח סעיף 11(ג) לחוק פיצויי פיטורים, התשכ\"ג-1963. הנתבעת לא ערכה לתובע גמר חשבון, לא שילמה לו את שכר חודש 9/2024 ואף לא הנפיקה לו תלוש לחודש זה, ולא שילמה לו דבר בגין זכויותיו."]),
      ...
      clause([{ t: "דמי מחלה ושכר חודש 9/2024: ", bold: true }, "התובע נעדר מעבודתו עקב מחלה ולא שולמו לו דמי מחלה, אף שלפי תלושי הנתבעת עצמה צבר 18.9 ימי מחלה שלא נוצלו, בניגוד לחוק דמי מחלה, התשל\"ו-1976. כמו כן, בגין עבודתו בחודש ספטמבר 2024, עד 28.9.2024, לא שולם לתובע שכר כלל."]),
      ...
      clause([{ t: "גמול שעות נוספות: ", bold: true }, "281 משמרות לפי התלושים × 4 שעות נוספות למשמרת (משמרת של 12 שעות) = 1,124 שעות × 30 ₪ × 137.5%, ממוצע התעריפים 125% ו-150%, = 46,365 ₪, בניכוי 8,545 ₪ ששולמו כשכר יסוד שעתי, ובסה\"כ ", { t: "37,820 ₪", bold: true }, ". החישוב נופל מחזקת 15 השעות הנוספות השבועיות שבסעיף 26ב לחוק הגנת השכר. לחלופין, ולכל הפחות, לפי רישומי הנתבעת עצמה בתלושים: 475 שעות נוספות, ובסה\"כ 11,049 ₪."]),
      ...
      clause([{ t: "דמי מחלה: ", bold: true }, "על דרך האומדן, 10 ימי היעדרות בשתי תקופות מחלה, השווים ל-6 ימי תשלום לפי סעיף 2 לחוק דמי מחלה × 286.44 ₪ = ", { t: "1,719 ₪", bold: true }, "."]),
      clause([{ t: "שכר חודש 9/2024: ", bold: true }, "בהעדר תלוש ורישום, לפי השכר הקובע = ", { t: "3,500 ₪", bold: true }, "."]),
      ...
      clause(["סך הכל על הנתבעת לשלם לתובע ", { t: "70,090 ₪", bold: true }, ", בצירוף הפרשי הצמדה וריבית כחוק ממועד סיום העבודה ועד התשלום בפועל. לחלופין, ובכפוף לשיקול דעת בית הדין הנכבד, בצירוף פיצויי הלנת שכר והלנת פיצויי פיטורים לפי חוק הגנת השכר."]),
