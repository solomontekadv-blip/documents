...
const intro = P([
  run("בית המשפט הנכבד מתבקש ליתן, בדחיפות ובטרם פקיעת הצו, את המבוקש בבקשה להארכת תוקף צו ההגנה ההדדי שהוגשה מטעם המבקשת (להלן: "),
  run("\"הבקשה\"", { bold: true }),
  run("), וזאת בהסכמת המשיב, אשר נמסרה בכתב, כמפורט להלן:"),
]);
...
const body = [
  clause(["בין הצדדים עומד בתוקפו צו הגנה הדדי, אשר הוארך בהסכמת הצדדים, במסגרת ההליך הקודם שהתנהל ביניהם (" + PREV_CASE + "), עד ליום 15.09.2026."]),
  clause(["בבקשה עתרה המבקשת להארכת תוקף הצו בשלושה חודשים נוספים, באותה מתכונת שבה עמד בתוקפו עד כה."]),
  clause(["הבקשה הומצאה ביום 20.08.2026 לעו\"ד ______________, אשר ייצגה את המשיב בהליך הקודם ואשר במסגרתו מסרה בשמו את הסכמתו להארכה הקודמת של הצו."]),
  clause(["עו\"ד ______________ עדכנה את הח\"מ כי ייפוי הכוח שבידיה ניתן לה לצורך ההליך הקודם בלבד, וכי משהבקשה נפתחה בתיק חדש שבו אינה רשומה כמייצגת, לא היה באפשרותה להגיש את תגובת המשיב באמצעות מערכת נט המשפט. זו, ורק זו, הסיבה שבגינה לא הוגשה תגובה כתובה בתיק, אף שהבקשה הגיעה לידי המשיב זה מכבר."]),
  clause(["ביום ______________ מסרה עו\"ד ______________ לח\"מ, בכתב, את הסכמת המשיב לבקשה, בזו הלשון:"]),
];
const body2 = [
  clause(["העתק הודעת ב\"כ המשיב מצורף להודעה זו ומסומן ", { t: "נספח א'", bold: true }, "."]),
  clause(["הנה כי כן, המשיב מסכים להארכת תוקף צו ההגנה ההדדי בשלושה חודשים נוספים, באותה מתכונת, לרבות המשך קיום הסדרי הראייה עם שלושת הקטינים במרכז הקשר \"פלא\" בטירת הכרמל, כפי שהתקיימו עד כה. אין אפוא כל מחלוקת בין הצדדים, והבקשה בשלה למתן החלטה."]),
  clause(["תוקף הצו הקיים פוקע ביום 15.09.2026. מתן ההחלטה בטרם מועד זה נדרש כדי למנוע פער בין פקיעת הצו הקיים לבין תחילת תוקפה של ההארכה, על כל המשתמע מכך לעניין ההגנה על המבקשת ועל הקטינים."]),
  clause(["אשר על כן, מתבקש בית המשפט הנכבד ליתן את המבוקש בבקשה ולהורות, בהסכמת הצדדים, על הארכת תוקף צו ההגנה ההדדי בשלושה חודשים נוספים, מיום 15.09.2026 ועד ליום 15.12.2026, באותה מתכונת שבה עמד בתוקפו עד כה, ותוך המשך קיום הסדרי הראייה עם הקטינים במרכז הקשר \"פלא\" בטירת הכרמל."]),
  clause(["למען הסדר הטוב, ומאחר שהמשיב אינו מיוצג בהליך זה, מתבקש בית המשפט הנכבד להורות על המצאת ההחלטה למשיב עצמו, לפי כתובתו הרשומה בתיק."]),
